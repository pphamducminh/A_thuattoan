...
void solve(){
    ...
    // check thang hang
    if (sign((AB^AC))==0 || sign((AB^AD))==0 || sign((AC^AD))==0 || sign((BC^BD))==0) {
 
        if (sign((AB^AC))==0 && sign((CA*CB))<=0) return cout << "YES\n", void();
        if (sign((AB^AD))==0 && sign((DA*DB))<=0) return cout << "YES\n", void();
        if (sign((AC^AD))==0 && sign((AC*AD))<=0) return cout << "YES\n", void();
        if (sign((BC^BD))==0 && sign((BC*BD))<=0) return cout << "YES\n", void();
 
        return cout << "NO\n", void();
    }
 
    // neu khong, check khac phia
    if (sign((AB^AC))*sign((AB^AD))<0 && sign((CD^CA))*sign((CD^CB))<0) return cout << "YES\n", void();
